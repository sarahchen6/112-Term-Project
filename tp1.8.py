...
affect reindeer (don’t worry, all of Santas are 
showing positive signs of recovery), and they 
can’t help Santa deliver the last 100 presents! 
Even though the reindeer told him the fastest way, 
...
next slide, and prove to the world that Santa’s 
still got what it takes to deliver presents to 
every last kid!'''
...
class InstructionsScreen(BackgroundScreen):
    def appStarted(mode):
        ...
        mode.instructionsText = '''Navigate Santa's sled through the maze by using the 'Up', 
...
Proceed to the next page to choose Santa’s sled and begin. 
...
class SledsScreen(BackgroundScreen):
    def appStarted(mode):
        mode.titleColor = 'black'
        mode.backButtonColor = mode.nextButtonColor = 'sea green'
        mode.backButtonTextColor = mode.nextButtonTextColor = 'white'
...
good excuse on the way though…'''
        mode.mode2Title = '''          Sled 2:
Will Pay To Take'''
        mode.mode2Text = '''Santa Will Pay you To Take 
this sled. Pros: not even the 
Grinch can be bothered by this 
sled, so he will not be chasing 
...
Christmas Corvette. This sled of 
...
if he chooses this sled - not only 
...
class GrinchMode(Maze):
    ...
    def checkSledGrinchIntersect(mode):
        sledX, sledY = Maze.getCell(mode, mode.dotX, mode.dotY)
        grinchX, grinchY = Maze.getCell(mode, mode.grinchX, mode.grinchY)
        if ((sledX, sledY) == (grinchX, grinchY)) and (mode.app.presents > 0):
    ...
    def moveGrinch(mode):
        ...
        grinchSol, grinchConn = getMazeSolutionConnections(mode.n, mode.mazeDict, (grinchCellX, grinchCellY), (sledX, sledY))
        try:
